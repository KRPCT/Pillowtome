...
provides:
  ...
  - Android ndk-context shim (Keyring.kt + MainActivity init + proguard keep) — code landed, AVD smoke DEFERRED (see Deferred section)
...
**The WebDAV connect spine is landed and wire-proven: keychain-only credentials (password-free IPC types, redacted Debug, zero logging), the D-95 http/self-signed gates, the D-97 classified Chinese errors with 503 backoff, a 7-test wiremock matrix that proves If-None-Match reaches the wire, the first three IPC commands on the shared SQLite pool — and a grounding discovery that fixes an Android keychain failure mode the plan's own spike would have hit. The AVD production-APK smoke is deferred to an end-of-phase manual batch (user decision).**
...
5. **Task 5 AVD checklist NOT run** — see Deferred section (orchestrator instruction; not a plan change).

## Deferred (Task 5 AVD production-APK smoke — user-deferred to end-of-phase manual batch)

The entire human checklist in 07-01-PLAN Task 5 is **deferred, not waived** — per orchestrator instruction the code edits landed now and the device gate runs in an end-of-phase manual batch. Until it passes, SYNC-01's Android half is code-complete but UNVERIFIED (RESEARCH Pitfall 1 gate):

- [ ] 1. AVD `Medium_Phone_API_36.1` running + lock-screen PIN set (Android Keystore prerequisite).
- [ ] 2. Host WebDAV server reachable at `http://10.0.2.2:<port>` from the emulator.
- [ ] 3. `pnpm tauri android build --debug --target x86_64 --apk` + `adb install -r` the universal debug APK (never `tauri android dev`).
- [ ] 4. force-stop → cold launch.
- [ ] 5. chrome://inspect IPC drive: (a) `allowHttp:false` → rejection containing 明文 HTTP and nothing persisted; (b) `allowHttp:true` → Ok; (c) `sync_get_config` → `configured:true` with NO password anywhere in the returned object.
- [ ] 6. **THE SPIKE:** force-stop → relaunch → `sync_get_config` still `configured:true` (keychain entry + config row survive process death). Watch logcat for ndk-context/Keystore errors. Failure = phase-blocking, per RESEARCH Pitfall 1.
- [ ] 7. `adb logcat -d | grep -i '<test password>'` returns nothing.
- [ ] 8. `sync_disconnect` → `configured:false`; persists across force-stop/relaunch.
- [ ] 9. Record device image, server used, per-step pass/fail in this file (update this section when run).

Also deferred with it: the plan's `<verify><automated>` APK build (`pnpm tauri android build --debug --target x86_64 --apk`) — the Android compile of the new Rust edges (`android-native-keyring-store`, `keyring-core`) and the Kotlin shim is only exercised by that build; desktop `cargo check/test` does not cover `cfg(target_os = "android")` code. This is the main residual risk of the deferral; the registration shim is small and mirrors the crate's own `cli` module pattern.
...
## Known Stubs
None in the shipped code paths. `SyncError::RemoteChanged` (412) is a deliberate seam consumed by 07-02's re-pull-merge-retry; `TransportConfig::password()` exists for 07-02/07-03 client construction from stored config. The Task 5 device verification is deferred (above), not stubbed.
...
- **Android gate:** the deferred Task 5 checklist above MUST pass before the phase can report done; the APK build is also the first Android compile of the new edges.

## Self-Check: PASSED (desktop side; Android device gate deferred)
...
- Task 5 AVD checklist — DEFERRED (see Deferred section); APK build verify deferred with it.
