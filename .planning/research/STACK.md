...
**Domain:** Cross-platform ebook reader (Desktop Win/macOS/Linux + Android), EPUB-first multi-format, CJK-superior typography, WebDAV/self-hosted sync, local-first — "Readest-level" capability, Chinese reading experience as the hard differentiator.
**Researched:** 2026-07-09
**Confidence:** HIGH (framework + EPUB engine + storage + sync); MEDIUM on later-phase PDF/MOBI performance specifics.

---

## TL;DR — Prescriptive Recommendation

**PRIMARY STACK: Tauri v2 (Rust core) + Web UI (React 19 + Vite + TypeScript) + foliate-js (EPUB/MOBI/KF8) + pdf.js (PDF) + SQLite via SQLx (tauri-plugin-sql) + WebDAV via `reqwest_dav` in the Rust core.**

This is the *only* framework family in which **both** the shared logic core (Rust) **and** the rendering engine (foliate-js running in the platform WebView) are genuinely written once and reused on desktop **and** Android — with best-in-class CJK typography that rides the WebView's CSS engine for free. It is the de-risked path: Readest ships exactly this shape (Next.js 16 + Tauri v2 + foliate-js) across all five platforms. We diverge from Readest only where it matters for us: **sync is WebDAV/self-hosted in Rust**, not Readest's proprietary cloud.

**Confidence this stack lets ONE core serve desktop + Android: HIGH.** See "Can one core serve both?" below.
...
**Latest Kotlin Readium toolkit: 3.2.0** (for the KMP alternative only). **pdf.js: `pdfjs-dist` 6.1.200.**

**MOBI/KF8 note:** foliate-js reads `.mobi`, `.azw3`, and combo files, splitting MOBI at `<mbp:pagebreak>` for performance; KF8 HUFF/CDIC decompression can be slow (known limitation) and embedded KF8 fonts are zlib-compressed (decompressed via `fflate`). Good enough for a later MOBI phase; if KF8 perf hurts, do decompression in the Rust core.

---

## Storage & Sync — evaluated

**Storage:** SQLite is the unanimous choice for local library + annotations + progress.
...
pnpm create tauri-app@latest    # choose: React, TypeScript, pnpm
...
pnpm add @tauri-apps/plugin-sql @tauri-apps/plugin-fs @tauri-apps/plugin-http @tauri-apps/plugin-dialog @tauri-apps/plugin-os
pnpm add pdfjs-dist@6.1.200 fflate
...
pnpm add -D typescript@5 vite @tauri-apps/cli@2.11.4
...
**If maximizing CJK/Chinese reading quality (our hard requirement):**
...
**If the product pivots to Android-first native (not our v1):**
...
---
*Stack research for: cross-platform CJK-first ebook reader (Pillowtome / 枕籍)*
*Researched: 2026-07-09*
